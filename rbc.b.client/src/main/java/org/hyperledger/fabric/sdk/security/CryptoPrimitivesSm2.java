...
public class CryptoPrimitivesSm2 implements CryptoSuite {
    ...
    public ECPublicKey getPublicKey(ECPrivateKey privateKey) throws Exception{
        ...
        ECPoint ecPoint1 = sm2.ecc_point_g.multiply(priS);
		ECPublicKeyParameters pubParams = new ECPublicKeyParameters(ecPoint1,sm2.ecc_bc_spec);
	...
	public String getCertCustomerNo(String strCert)
    {
	 ...
	 ASN1Sequence seq = null;
		...
		try {
			...
			X509CertificateStructure cert = new X509CertificateStructure(seq);
			TBSCertificateStructure certStruct = cert.getTBSCertificate();
			X500Name xName = certStruct.getSubject();
			String customerNo = xName.getRDNs()[0].getFirst().getValue().toString();
			aIn.close();
			return customerNo;
