...
public class TransactionContext {
    ...
    public TransactionContext(Channel channel, User user, CryptoSuite cryptoSuite) {
        ...
        //修改成标准的Hash SHA256
        byte[] txh = Hash.hash(comp.toByteArray());
    ...
    byte[] sign(byte[] b) throws CryptoException {
    	...
    	if (user.getName() != null ){
    		userId = user.getName().getBytes();
    	}
        return cryptoSuite.sign(getUser().getEnrollment().getKey(),userId, b);
    }
    ...
    public ByteString[] signByteStrings(User[] users, ByteString... bs) throws CryptoException {
        ...
        for (User user : users) {
            ...
            ret[++i] = ByteString.copyFrom(cryptoSuite.sign(user.getEnrollment().getKey(),userId, signbytes));
        }
        return ret;
    }
