 ...
 * @see Block
 */
public class BlockEvent extends BlockInfo implements BaseEvent{
//    private static final Log logger = LogFactory.getLog(BlockEvent.class);
    ...
    BlockEvent(EventHub eventHub, Event event) throws InvalidProtocolBufferException {
    	...
    	if (event.getEventCase().getNumber() == 4)//交易失败的通知 REJECTION
    	{
    	}
    ...
    TransactionEvent getTransactionEvent(int index) throws InvalidProtocolBufferException {
        ...
        return new TransactionEventImpl((TransactionEnvelopeInfo) getEnvelopeInfo(index), index);
    }

    public class TransactionEventImpl extends TransactionEnvelopeInfo implements TransactionEvent{

    	TransactionEventImpl(TransactionEnvelopeInfo transactionEnvelopeInfo, int index) {
