        ...
        StreamObserver<PeerEvents.Event> eventStream = new StreamObserver<PeerEvents.Event>() {
            ...
            public void onNext(PeerEvents.Event event) {
                ...
                if (event.getEventCase() == PeerEvents.Event.EventCase.BLOCK) {
                    try {
                       eventQue.addEvent(new BlockEvent(EventHub.this, event));  //add to channel queue
                ...
                if (event.getEventCase() == PeerEvents.Event.EventCase.REJECTION) {

                    eventQue.addEvent(new RejectionEvent(EventHub.this, event));  //add to channel queue
...
//        });
    ...
    private void blockListen(TransactionContext transactionContext) throws CryptoException {
        ...
        PeerEvents.Register register = PeerEvents.Register.newBuilder()
                .addEvents(PeerEvents.Interest.newBuilder().setEventType(PeerEvents.EventType.BLOCK).build()).addEvents(PeerEvents.Interest.newBuilder().setEventType(PeerEvents.EventType.REJECTION).build()).build();
