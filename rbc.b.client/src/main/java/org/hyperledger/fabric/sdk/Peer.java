...
public class Peer {
     ...
     */
    FabricProposalResponse.ProposalResponse sendExecTransaction(String chainId,String txId,FabricProposal.SignedProposal proposal)
            throws PeerException, InvalidArgumentException {
        checkSendProposal(proposal);
        ...
        RBCMessage rbcMessage = RBCMessage.newBuilder()
        		...
        		.setChainID(chainId)
        		.setTxID(txId)
        		.setData(proposal.toByteString())
        		.build();
        try {
            return localEndorserClient.sendExecTransaction(rbcMessage);
        } catch (Throwable t) {
            endorserClent = null;
            throw t;
        }
    ...
    * @throws InvalidArgumentException
    */
    public Ab.BroadcastResponse sendMessage(String  chainId,RBCMessage rbcMessage)
           throws Exception {
       logger.debug(format("peer.send envope name: %s, url: %s", name, url));

       Ab.BroadcastResponse bcRes = null;
       EndorserClient localEndorserClient = endorserClent; //work off thread local copy.
       ...
       RBCMessage rbcMessage = RBCMessage.newBuilder()
       		.setType(0)
       		.setChainID(chainId)
       		.setData(env.toByteString())
       		.build();
       		*/
       try {
    	   FabricProposalResponse.ProposalResponse res =  localEndorserClient.sendExecTransaction(rbcMessage);
    	   
    	   if (res.getResponse().getStatus() == 200){
    		   bcRes = Ab.BroadcastResponse.parseFrom(res.getPayload());
    	   }
       } catch (Throwable t) {
           endorserClent = null;
           throw t;
    ...
    public static Peer createNewInstance(String name, String grpcURL, Properties properties) throws InvalidArgumentException {
